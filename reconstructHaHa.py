...
def filter_points(
    ...
    """
    x_min = x_full * x_percent
    x_max = x_full * (1 - x_percent)
    y_min = y_full * y_percent
    y_max = y_full * (1 - y_percent)
...
def group_and_draw_circles_MAIN(ptls:List[Tuple[int, int]],orig_w,orig_h, img: np.ndarray, x_pct: float, y_pct: float, r: int) -> np.ndarray:
    ...
    OBJ_group_and_draw_circles = downsample480andMaskSide(ptls, orig_w, orig_h, img, x_pct, y_pct, r)
    # step2 
...
def downsample480andMaskSide(ptls:List[Tuple[int, int]],w,h, img: np.ndarray, x_pct: float, y_pct: float, r: int) -> STRUCT_group_and_draw_circles:
